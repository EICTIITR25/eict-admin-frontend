...
import AddCoursePage from "../FDPCourse/AddCoursePage";
...
const ShortTermTrainingPage = () => {
  ...
  //CREATE
  const { mutate: createShortTermCourse } = useCreate(
  ...
  const { mutate: createFAQ } = useCreate(
    `/courses/short-term-courses/${courseId}/faqs/`,
    [`/courses/short-term-courses/${courseId}/faqs/`, "{}"],
    {
      onSuccess: (data) => {
  ...
  const { data: product_code = {} } = useFetch(
    !courseId ? `/courses/latest-product-codes?category_id=5` : "",
    {},
    {
      enabled: !!!courseId,
      retry: false,
  ...
  const { data: featureList = [] } = useFetch(
    `/courses/features/?type=short-term-training`,
    {},
    {
      retry: false,
    }
  ...
  const { data: courseListById = {} } = useFetch(
    courseId ? `/courses/short-term-courses/${courseId}/` : "",
    {},
    {
      enabled: !!courseId,
      retry: false,
  ...
  const { mutate: updateSTT } = useUpdate(
    `/courses/short-term-courses/${courseId}/`,
    [`/courses/short-term-courses/${courseId}/`, "{}"],
    {
      onError: (error) => {
        getErrorMessage(error);
      ...
      onSuccess: () => {
        toast.success("Short term training Course Updated Successfully.");
  ...
  const goToNextStep = () => {
    ...
    if (step === 1 && !selectedCourse?.base_course_id) {
      ...
      const formData = buildFormData(form, keyMap, extraFields);
      createShortTermCourse(formData);
    } else {
      setStep((prev) => prev + 1);
    }
  };
  ...
  const handleSubmit = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (isEditMode && Object.keys(editedFields).length > 0) {
      ...
      toast.success("Short term training Course Created Successfully.");
      navigate("/course-management/all-courses");
    }
  };
  ...
  useEffect(() => {
    if (selectedCourse) {
      const selectedCourseIds =
        selectedCourse?.tags?.map((item: any) => item.id) || [];
      const selectedFacultyId =
        selectedCourse?.course_faculty?.map((item: any) => item.id) || [];
      const initialForm = {
        name: selectedCourse.title || "",
        course_sub_headings: selectedCourse?.sub_heading || "",
        deadline:
    ...
    <>
      {step === 1 && (
        <div className="admin_panel step1">
          <div className="Breadcrumbs">
            <h3>Add Course 1/3 (Short Term Training)</h3>
      ...
      {step === 2 && (
        <div className="admin_panel step2">
          <div className="Breadcrumbs">
            <h3>Add Course 2/3 (Short Term Training)</h3>
            <div className="btn_grp">
              <button
                className="btn"
      ...
      {step === 3 && (
        <div className="admin_panel step2">
          <div className="Breadcrumbs">
            <h3>Add Course 3/3 (Short Term Training)</h3>
            <div className="btn_grp">
              <button
                className="btn"
...
export default ShortTermTrainingPage;
